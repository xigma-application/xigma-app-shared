...
description: At the end of any task in this repo that leaves file changes, show the user a proposed git commit message (must start with "XG-SHARED: ") and ask for approval before touching git. Load right before your closing summary whenever `git status` isn't clean. The user's approval of the shown message authorizes commit AND push together — no separate push confirmation needed after that.
---

# xigma-app-shared: commit-message approval, then commit + push
...
XG-SHARED: <short, why-focused summary of what changed>
```
...
Every message starts with `XG-SHARED: ` — this repo's convention (mirrors `xigma-app`'s
`XG-APP: `, adapted to this repo's own name). A drafted message missing the prefix is a mistake —
fix it before showing the form, don't rely on the user to notice.

## Language

The commit message itself (everything after `XG-SHARED: `) is always written in English, even when
...
if their edit drops the `XG-SHARED: ` prefix, say so rather than silently re-adding it yourself.
