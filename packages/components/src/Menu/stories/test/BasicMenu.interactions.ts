...
export const playBasicMenu = async ({ canvasElement }: TPlayContext): Promise<void> => {
  ...
  await waitFor(() => expect(screen.queryByRole('menuitem', { name: 'Undo' })).not.toBeInTheDocument());
};

export const playRichMenu = async ({ canvasElement }: TPlayContext): Promise<void> => {
  const canvas = within(canvasElement);
  ...
  const subItem = await screen.findByRole('menuitem', { name: 'Rotate 90°' }, { timeout: 3000 });
  await expect(subItem).toBeVisible();

  // after: close so the rest of the page is no longer aria-hidden
  await userEvent.keyboard('{Escape}');
  await userEvent.keyboard('{Escape}');
  await waitFor(() => expect(screen.queryByRole('menuitem', { name: /Transform/ })).not.toBeInTheDocument());
};
